...
func ruleAllowsOrigin(allowedOrigin string, browserOrigin string) bool {
	...
	if err != nil {
		log.Println("Error parsing allowed origin specification", "spec", allowedOrigin, "error", err)
		return false
	}
	browserScheme, browserHostname, browserPort, err = parseOriginURL(browserOrigin)
	if err != nil {
		log.Println("Error parsing browser 'Origin' field", "Origin", browserOrigin, "error", err)
		return false
	}
